export default function NoticeContent({ entry, icon, ...props }) {
  ...
  return (
    <span className="maint-notice__segment" {...props}>
      ...
      {!isUpcoming && entry.showEstimatedReturn && entry.estimatedReturn && (
        <span className="maint-notice__eta">
          Estimated return: {entry.estimatedReturn}
        </span>
      )}
      {entry.siteUrl && (
        <a
          className="maint-notice__link"
          href="https://status.httphive.com"
          target="_blank"
